...
function AddInfoSerie($info)
{
    ...
    return '<table style="width: 100%; border:solid 1px #999999;" cellspacing="0">
        <tr>
            <td class="tdSerie">'.$info["SERIE"].'</td>
        </tr>
        </table>
...
function AddAlbumDetaille($infos)
{
    ...
    $html .= "\n".'<tr><td align="center" class="tdNumTome">'.$infos["NUM_TOME"].'</td>';
    $html .= "\n".'<td class="tdTitre" colspan="2">'.$infos["TITRE"].'</td></tr>';

    // Détermine l'echelle à utiliser
    $html .= "\n".'<tr><td class="tdImg" rowspan="8">';


        $html .= "\n".'<img src="'.BDO_URL_COUV.$infos["IMG_COUV"].'" border="0" width="100"></td></tr>';

        /*$html .= "\n".'Image</td></tr>';*/

    unset($infos["TITRE"]);
    unset($infos["NUM_TOME"]);
    unset($infos["IMG_COUV"]);

    foreach ($infos as $lib => $data) {
...
<style type="text/css">
<!--
.tdImg     { width:105px; vertical-align:top; text-align:center;}
.tdNumTome     { font-size:12px; width:105px; vertical-align:top; text-align:center; font-weight:bold;}
.tdTitre     { font-size:12px; width:255px; vertical-align:top; text-align:left; font-weight:bold;}
.tdLib     { font-size:11px; width:65px; vertical-align:top; text-align:right;}
.tdData    { font-size:11px; width:190px; vertical-align:top; text-align:left;}
...
<page backtop="10mm" backbottom="10mm" style="font-size: 12pt">
    <page_header>
...
while ($DB->next_record() and ($num<300))
{
    ...
    */
    ...
    if ($col == 1) {
        echo '<table cellpading="0" cellspacing="2">';
        echo '<tr>';
    }
    ...
    echo AddAlbumDetaille($info);
    echo '</td>';
    if ($col == 2) {
        echo '</tr></table>';
        $col = 1;

        if ($countForDiv==2) echo '</div>';
    }
    else {
        $col = 2;
    }
