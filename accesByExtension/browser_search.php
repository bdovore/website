...
if ($rb_browse == 'ser' || !$rb_browse) {
	...
	if ($let) {
		...
		*/
			$query_where = "WHERE nom like '".PMA_sqlAddslashes($let,true)."%' ";
		//}
	}
	else {
		$query_where = "";
...
elseif ($rb_browse == 'aut') {
	...
	if ($let) {
		$query_where = "WHERE pseudo like '".PMA_sqlAddslashes($let,true)."%'";
	}
	else {
		$query_where = "";
	}
...
elseif ($rb_browse == 'genr') {
	...
	if ($let) {
		$query_where = "WHERE LIBELLE like '".PMA_sqlAddslashes($let,true)."%'";
	}
	else {
		$query_where = "";
	}
...
else if (!empty($totalRows))
{
	...
	if ($pageNum > 0)
	$t->set_var("URLPREVPAGE",'<a href="javascript:document.browser.pageNum.value=\''.max(0, $pageNum - 1).'\';go1()">Pr&eacute;c&eacute;dent</a>');
	else
	$t->set_var("URLPREVPAGE",'');
	
	if ($pageNum < $totalPages)
	$t->set_var("URLNEXTPAGE",'<a href="javascript:document.browser.pageNum.value=\''.min($totalPages, $pageNum + 1).'\';go1()">Suivant</a>');
	else
	$t->set_var("URLNEXTPAGE",'');
...
$t->set_var("TITRESEARCH","<b>Commence par '<i>" . htmlspecialchars($let) . "</i>...'</b>");
