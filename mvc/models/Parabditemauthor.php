...
class Parabditemauthor extends ParabdDbLine
{
    ...
    public function forItem($itemId) { return $this->fetchAllQuery("SELECT l.*,COALESCE(NULLIF(a.PSEUDO,''),NULLIF(TRIM(CONCAT_WS(' ',a.PRENOM,a.NOM)),''),CONCAT('Artiste #',l.AUTHOR_ID)) LABEL FROM parabd_item_author l LEFT JOIN bd_auteur a ON a.ID_AUTEUR=l.AUTHOR_ID WHERE l.ITEM_ID=" . intval($itemId)); }
    ...
    public function defaultRoleForAuthor($authorId) { $row = $this->fetchOneQuery('SELECT FLG_DESSIN,FLG_DESIGN,FLG_SCULPT,FLG_PEINT FROM bd_auteur WHERE ID_AUTEUR=' . intval($authorId)); return $row ? ParabdRules::defaultAuthorRole($row) : ''; }
    public function addForItem($itemId, array $row, $userId) { if (!$this->fetchOneQuery('SELECT ID_AUTEUR FROM bd_auteur WHERE ID_AUTEUR=' . intval($row['id']))) throw new ParabdException('VALIDATION_ERROR', 'Artiste inconnu.'); (new self())->persist(array('ITEM_ID' => intval($itemId), 'AUTHOR_ID' => intval($row['id']), 'ROLE' => $row['role'], 'CREATED_BY' => intval($userId))); }
