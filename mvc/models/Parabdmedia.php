...
class Parabdmedia extends ParabdDbLine
{
    ...
    public function findForItem($itemId, $mediaId, $forUpdate = false) { return $this->fetchOneQuery('SELECT * FROM parabd_media WHERE ID_MEDIA=' . intval($mediaId) . ' AND ITEM_ID=' . intval($itemId) . ($forUpdate ? ' FOR UPDATE' : '')); }
    public function deleteForItem($itemId, $mediaId) { $this->executeQuery('DELETE FROM parabd_media WHERE ID_MEDIA=' . intval($mediaId) . ' AND ITEM_ID=' . intval($itemId)); return Db_affected_rows($this->connection()) === 1; }
    public function firstVisibleForItem($itemId) { return $this->fetchOneQuery('SELECT ID_MEDIA FROM parabd_media WHERE ITEM_ID=' . intval($itemId) . ' AND IS_HIDDEN=0 ORDER BY SORT_ORDER,ID_MEDIA LIMIT 1'); }
